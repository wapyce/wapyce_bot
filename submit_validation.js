...
for (var i = 0, length = completeValidations.length; i < length; i++) {
    completeIssues = completeIssues.concat(completeValidations[i]['issues']);
}

function finishValidation() {
    var options = {
        method: 'PUT',
        url: FINISH_VALIDATION_PATH.replace('%s', wapyceValidationUUID),
        headers: HEADERS
    };
    request(options, function(error, response, body) {
        ...
        if (response.statusCode === 200) {
            ...
            fs.renameSync(
                ...
                path.join(
                    validationDirectory,
                    path.basename(config.default_request_validation_file)
                )
            );
            fs.renameSync(
                pagesFile,
    ...
    if (!validation.hasOwnProperty('uuid')) {
        ...
        request(options, function(error, response, body) {
            ...
            if (response.statusCode === 201) {
                ...
                completeValidations[index]['uuid'] = wapycePage['uuid'];
                var issues = completeValidations[index]['issues']
                for (var i = 0, length = issues.length; i < length; i++) {
